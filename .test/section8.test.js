...
describe("test countArrayLength function", () => {
  test("case: [1, 3, 10]", () => {
    expect(countArrayLength([1, 3, 10])).toBe("この配列は3個のデータを持っています");
  });
  test("case: []", () => {
    expect(countArrayLength([])).toBe("この配列は空配列です");
  });
  test('case: [true, "a", "b", null]', () => {
    expect(countArrayLength([true, "a", "b", null])).toBe("この配列は4個のデータを持っています");
  });
  test("case: [undefined]", () => {
    expect(countArrayLength([undefined])).toBe("この配列は1個のデータを持っています");
  });
  test("case: 14", () => {
